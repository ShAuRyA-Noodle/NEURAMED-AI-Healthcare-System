...
const NAV_ITEMS = [
  ...
  { path: '/sessions', label: 'Sessions', icon: Activity, badge: true },
];

const Sidebar = ({ isOpen, onClose }: { isOpen?: boolean, onClose?: () => void }) => {
  const location = useLocation();

  // Session count badge — count new sessions since last visit
  const [lastVisitTime] = useState(() => {
    const stored = localStorage.getItem('neuramed-last-sessions-visit');
    return stored || new Date(0).toISOString();
  });

  // Update last visit when navigating to sessions
  useEffect(() => {
  ...
  // Query total sessions for badge
    ...
    <div className={`sidebar-container ${isOpen ? 'open' : ''}`} style={{
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '0 12px', marginTop: 24, flex: 1 }}>
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link key={item.path} to={item.path} data-cursor="hover"
              style={{
