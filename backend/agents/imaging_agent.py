...
from ml import cxr_classifier, dicom_io
...
def _create_annotated_image(img, gray, scan_type: str) -> tuple:
    """Create an OpenCV VISUAL OVERLAY only.

    This draws boxes over high-contrast blobs purely so the user has something
    to look at. It deliberately produces NO clinical signal — no confidence,
    no mm sizes, no anomaly decision. Those come from the real models
    (cxr_classifier / vision LLM). Returns (annotated_bytes, overlay_regions).
    """
    ...
    for i, cnt in enumerate(contours):
        ...
        # Neutral display color — no red/anomaly semantics
        color = (0, 255, 255)
        cv2.rectangle(annotated, (x, y), (x + w, y + h), color, 2)
        cv2.putText(annotated, f"R{i + 1}", (x, y - 5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)

        # Overlay geometry ONLY — pixels, not clinical measurement.
        regions.append({
            "id": i + 1,
            "overlay_box": {"x": x, "y": y, "w": w, "h": h},
            "area": int(area),
        })
    ...
    return annotated_bytes, regions
...
def analyze(
    ...
    # ── 1. DICOM vs raster: get a grayscale array + HONEST metadata ──
    if dicom_io.is_dicom(image_bytes):
        gray_float, meta = dicom_io.read_dicom(image_bytes)
        gray = np.clip(gray_float, 0, 255).astype(np.uint8)
        img = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
        # DICOM bytes aren't browser-viewable — re-encode a PNG for display + vision
        _ok, _png = cv2.imencode('.png', gray)
        display_bytes = _png.tobytes()
    else:
    ...
    measurements_enabled = bool(meta.get("measurements_enabled"))

    original_b64 = bytes_to_b64(display_bytes)

    # OpenCV overlay — VISUAL ONLY, drives no clinical decision
    annotated_bytes, regions = _create_annotated_image(img, gray, scan_type)
    ...
    # ── 2. Is the real CXR classifier applicable to this modality? ──
    st_norm = (scan_type or "").lower().replace("-", "").replace(" ", "")
    is_cxr_modality = ("xray" in st_norm) or ("radiograph" in st_norm) or (st_norm in ("cr", "dx"))
    classifier_available = is_cxr_modality and "chest" in (body_region or "").lower()
    ...
    if classifier_available:
        try:
            pathology_scores = cxr_classifier.classify(gray)
            classifier_ran = bool(pathology_scores)
            top_findings = sorted(pathology_scores.items(), key=lambda kv: kv[1], reverse=True)[:5]
        except Exception as e:
            logger.error(f"CXR classifier failed: {e}")
            pathology_scores, top_findings, classifier_ran = {}, [], False

    # ── 3. Ground the vision LLM in the classifier's real numbers ──
    effective_indication = clinical_indication or "general screening"
    if classifier_ran and top_findings:
        findings_str = ", ".join(f"{name} {prob:.2f}" for name, prob in top_findings)
        effective_indication = (
            f"{effective_indication}\n\n"
            f"A validated CXR classifier (TorchXRayVision {cxr_classifier.model_name()}) "
            f"reports: {findings_str}. Write findings consistent with these probabilities; "
            "do not contradict them without stating why."
        )

    # ── 4. Vision analysis on the actual pixels ──
    vision_result = analyze_scan_with_vision(
        display_bytes, scan_type, body_region,
        effective_indication, patient_age, patient_gender
    ) or {}

    vision_usable = bool(
        vision_result.get("clinical_impression")
        or vision_result.get("findings")
        or vision_result.get("impression")
    )

    # ── 5. FAIL LOUD: no real inference of any kind ──
    if not classifier_ran and not vision_usable:
        raise InferenceUnavailable(
            "No real imaging inference available (no CXR classifier for this "
            "modality and vision model unavailable)",
            vendor="imaging",
        )

    processing_ms = int((time.time() - start) * 1000)

    # ── 6. Confidence: classifier max-prob for CXR; else the vision LLM's own ──
    if classifier_ran and pathology_scores:
        overall_confidence = float(max(pathology_scores.values()))
        anomaly_detected = overall_confidence > 0.5
    else:
        vision_conf = vision_result.get("confidence_score") or vision_result.get("confidence")
        overall_confidence = float(vision_conf) if isinstance(vision_conf, (int, float)) and vision_conf > 0 else 0.1

        # Anomaly decision from the LLM's medical assessment (never OpenCV)
        anomaly_detected = False
        llm_assessment = (vision_result.get("overall_assessment") or "").lower()
        ...
        llm_urgency = (vision_result.get("urgency") or "routine").lower()
    ...
    # ── 7. Provenance envelope (real result => status OK) ──
    if classifier_ran:
        grounded = [f"vision:{vision_result.get('_vision_model')}"] if vision_usable else []
        prov = Provenance(
            status=InferenceStatus.OK, source="real_model",
            model=cxr_classifier.model_name(), vendor="torchxrayvision",
            grounded_in=grounded,
        )
        disclaimer = cxr_classifier.DISCLAIMER
    else:
        vision_model = vision_result.get("_vision_model") or VISION_MODELS[0]
        prov = Provenance(
            status=InferenceStatus.OK, source="real_model",
            model=vision_model, vendor="groq",
        )
        disclaimer = ""
    provenance_dict = wrap_result(vision_result, prov)["provenance"]

    # ── 8. Measurements honesty: no spacing => no mm ──
    if measurements_enabled:
        measurements_text = _format_measurements(vision_result.get("measurements", ""))
    else:
        measurements_text = ""
